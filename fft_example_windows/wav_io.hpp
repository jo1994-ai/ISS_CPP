...
// Samples are returned as doubles in [-1, 1], interleaved (n_samples, n_chan).
// Writes 16-bit PCM.
#pragma once
...
struct WavData {
  ...
  size_t n_chan = 0;
  size_t n_samples = 0;
  std::vector<double> samples;  // interleaved, (n_samples, n_chan)
};
...
inline void write_wav(const std::string& filename, const std::vector<double>& samples,
  ...
  for (double v : samples) {
    ...
    long s = std::clamp(std::lround(v * 32768.0), -32768L, 32767L);
