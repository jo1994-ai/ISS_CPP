 ...
 public:
  AuxIVAIPOnline(size_t n_freq, size_t n_chan, size_t n_iter = 3,
      ...
      : OnlineAuxIVABase(n_freq, n_chan, n_iter, alpha, model, eps),
        WV_(size_t(max_threads()), std::vector<cplx>(n_chan * n_chan)),
        w_(size_t(max_threads()), std::vector<cplx>(n_chan)) {}

 protected:
  // Frequency bins are independent: each solve only touches its own bin,
  // so this loop parallelizes without any locking.
...
#pragma omp parallel for schedule(static) if (parallel_)
    ...
    for (long long ff = 0; ff < (long long)n_freq_; ff++) {
      size_t f = size_t(ff);
      cplx* WV = WV_[size_t(this_thread())].data();  // scratch, per thread
      cplx* w = w_[size_t(this_thread())].data();
      ...
      for (size_t c = 0; c < n; c++) w[c] = (c == k) ? 1.0 : 0.0;
      if (!solve_inplace(WV, w, n)) continue;

      // normalize by sqrt(w^H V_k w)
      double denom = 0.0;
      for (size_t r = 0; r < n; r++) {
        cplx Vw = 0.0;
        for (size_t c = 0; c < n; c++) Vw += Vk[r * n + c] * w[c];
        denom += std::real(std::conj(w[r]) * Vw);
      }
      double scale = 1.0 / std::sqrt(std::max(denom, eps_));

      // store back as a row, i.e. as w_k^H
      for (size_t c = 0; c < n; c++) W_[w_idx(f, k, c)] = std::conj(w[c] * scale);
    }
  }

 private:
  std::vector<std::vector<cplx>> WV_, w_;
};
