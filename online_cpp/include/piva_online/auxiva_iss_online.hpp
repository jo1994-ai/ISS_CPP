 ...
 public:
  AuxIVAISSOnline(size_t n_freq, size_t n_chan, size_t n_iter = 3,
      ...
      : OnlineAuxIVABase(n_freq, n_chan, n_iter, alpha, model, eps),
        wk_(size_t(max_threads()), std::vector<cplx>(n_chan)),
        v_(size_t(max_threads()), std::vector<cplx>(n_chan)) {}

 protected:
  // Frequency bins are independent: W[f] and V[.][f] are only touched by
  // their own bin, so this loop parallelizes without any locking.
...
#pragma omp parallel for schedule(static) if (parallel_)
    ...
    for (long long ff = 0; ff < (long long)n_freq_; ff++) {
      size_t f = size_t(ff);
      cplx* wk_buf = wk_[size_t(this_thread())].data();  // scratch, per thread
      cplx* v_buf = v_[size_t(this_thread())].data();

      // frozen pivot row (stores w_k^H)
      for (size_t c = 0; c < n; c++) wk_buf[c] = W_[w_idx(f, k, c)];
      ...
      for (size_t m = 0; m < n_src_; m++) {
        ...
        for (size_t r = 0; r < n; r++) {
          ...
          for (size_t c = 0; c < n; c++) Vw += Vm[r * n + c] * std::conj(wk_buf[c]);
          denom += std::real(wk_buf[r] * Vw);
          numer += W_[w_idx(f, m, r)] * Vw;
        }
        denom = std::max(denom, eps_);
        v_buf[m] = (m == k) ? cplx(1.0 - 1.0 / std::sqrt(denom)) : numer / denom;
      }

      for (size_t m = 0; m < n_src_; m++)
        for (size_t c = 0; c < n; c++) W_[w_idx(f, m, c)] -= v_buf[m] * wk_buf[c];
    }
  }

 private:
  std::vector<std::vector<cplx>> wk_, v_;
};
